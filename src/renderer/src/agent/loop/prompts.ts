...
- The closing summary is visible assistant text OUTSIDE <think>. One closer after success, then STOP. "npm install succeeded, now I run npm run dev" is NOT a closer — call the tool instead. If PREVIEW_URL / Local: http://127.0.0.1:… already printed with exit_code=0, do not rerun npm — that is success. If the command failed, fix and retry, then one closer and STOP.
...
- A plan row that names a file to create is not done until that path is written. Do not rewrite an already-complete file to dodge the next missing file. Opening preview is not a substitute for writing it.
...
- Missing executable (not recognized / not in PATH): do NOT winget, choco, scoop, or download SDKs/archives. Use a tool already on PATH. If none works, name it and stop.
...
- Compiler stacks (Node/Python/Go/Java/Rust/.NET/C/C++): "done" only after a successful patch AND one verify command with exit_code=0.
