...
- Unclear repo layout: call explore_subagent (read-only) before large edits. If the user already pasted product facts and GitHub URLs, skip explore / web_search / curl — write files immediately.
...
- Facts from GitHub/README: if the user already pasted product facts AND GitHub URLs, NEVER call web_search, explore_subagent, or curl GitHub — write CSS/JS/HTML from those facts. Search only when a required URL is missing AND facts are not already in the message. FORBIDDEN: git clone of the product repo / into /tmp, curl/wget of raw.githubusercontent README, Get-ChildItem -Recurse, repeated Test-Path, Copy-Item from a clone.
- If the detected stack is HTML: no build/test; verify_project is a one-shot entry check or Start-Process (Resolve-Path .\\index.html) ONCE. i18n values MUST be strings (never objects/arrays in textContent — that is "[object Object]"). NEVER get_diagnostics on static HTML. NEVER open the LLM API port as the site. Vite/npm run dev → the printed Local: URL.
  Multi-file landing: ONE complete write_file per path, then STOP. data-i18n tags MUST contain visible default-language text (JS only swaps language — never empty <h1 data-i18n>). CSS class names are a contract: index.html must use the classes from THIS styles.css (the names listed after the CSS write). Inline <svg> MUST have width and height. After index.html exists, JS getElementById / data-i18n keys MUST match it. Before that, missing #ids are expected — write CSS/HTML next, never rewrite JS to chase markup that is not on disk yet. After CSS+HTML+JS are on disk, Start-Process once — do not rewrite js/main.js. Do not open preview as "done" while LANDING_CONTRACT / EDIT_SANITY is open.
